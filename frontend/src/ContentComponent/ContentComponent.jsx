...
function ContentComponent(props) {
  ...
  const showButtonDelete = (event, index, idImage) => {
    const indexElement =
      event.target.parentElement.children[1].classList.value.split("-")[2];
    if (indexElement === index) {
      ...
      setElementDelete(
        <DeleteComponent
          number={idImage}
          getlengthImageNew={getlengthImageNew}
        />
      );
    }
  };
  ...
  return (
    <div className={"content w-full min-h-screen relative"}>
      <div className="cover-content w-11/12 justify-center gap-2 mx-auto">
        {dataImages.length === 0 ? (
          ...
          dataImages.map((element, index) => (
            <div
              onMouseEnter={(event) =>
                showButtonDelete(event, index, element._id)
              }
              onMouseLeave={hideButtonDelete}
              className={`banner w-52 h-max relative overflow-hidden`}
            >
              <img
                className={"rounded-lg image"}
                src={`http://127.0.0.1:4000/${element.filePath}`}
                alt=""
              />
              <div className={`delete-cover-${index}`}>
                {elementIndex === index ? elementDelete : ""}
              </div>
            </div>
          ))
        )}
      ...
      <div
        className={`upload-image md:w-5/12 w-4/12 h-96 absolute inset-0 z-50 rounded-lg bg-white mx-auto grid justify-center content-center ${
          status === "show" ? "" : "hidden"
        }`}
      >
        <svg
        ...
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className={`sm:w-16 sm:h-16 h-32 w-32 mx-auto`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
        ...
        <div
          className={`w-full h-60 ${
            showPreviewImage ? "" : "hidden"
          } rounded-t-lg absolute inset-0`}
        >
          <img
            src={imageTest}
            ref={imgRef}
        ...
        <p className={`text-center ${showPreviewImage ? "hidden" : ""}`}>
          Upload Foto Disini
        </p>
        <div className="cover-upload grid justify-center content-center">
          <input
            ...
            ref={elementRef}
            onChange={(event) => getDetailFile(event.target)}
            type="file"
            hidden={true}
          />
          <p
            className={`text-xs mt-3 text-red-600 ${
              validationImage ? "" : "hidden"
            }`}
          >
        ...
        <div className="cover-upload grid justify-center content-center">
          <button
            onClick={up}
            className={`p-2 bg-red-500 mt-2 flex gap-1 text-white rounded-lg ${
              uploadButton ? "" : "hidden"
            } ${validationImage ? "hidden" : ""}`}
            type={"button"}
          >
      ...
      <div
        className={`flex justify-center items-center gap-2 navigation rounded-3xl bg-white h-14 fixed top-3/4 bottom-2/4 w-52 2xl:hidden md:w-5/12 sm:w-11/12`}
      >
        <div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
